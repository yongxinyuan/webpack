...
class Compiler {
	...
	run(callback) {
		...
		// 定义运行函数
		const run = () => {
			// compiler.hooks.beforeRun 都有哪些插件执行了
			// ProgressPlugin 处理进度
			// NodeEnvironmentPlugin 处理文件系统
			this.hooks.beforeRun.callAsync(this, err => {
				if (err) return finalCallback(err);

				// compiler.hooks.run 都有哪些插件可能执行？？？
				// ProgressPlugin 处理进度
...
${other}`);
	...
	compile(callback) {
		...
		// Compiler.hooks.beforeCompile 都有哪些插件可能执行？？？
		// DllReferencePlugin 处理manifest?
		// LazyCompilationPlugin
		this.hooks.beforeCompile.callAsync(params, err => {
			if (err) return callback(err);

			// Compiler.hooks.compile 都有哪些插件可能执行？？？
			// ExternalsPlugin 监听 NormalModuleFactory.hooks.factorize 处理忽略的文件
			// DelegatedPlugin 这是导出的插件？
			// DllReferencePlugin 这是导出的插件？
			...
			// Compiler.hooks.make 都有哪些插件可能执行？？？
			// AutomaticPrefetchPlugin 导出插件？
			// DllEntryPlugin
			// DynamicEntryPlugin
			// EntryPlugin 这里开始处理入口
			// PrefetchPlugin
			// ProgressPlugin
			// ContainerPlugin
			this.hooks.make.callAsync(compilation, err => {
				...
				// Compiler.hooks.finishMake 都有哪些插件可能执行？？？
				// ProgressPlugin
				// ProvideSharedPlugin
