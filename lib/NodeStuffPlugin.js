...
class NodeStuffInWebError extends WebpackError {
	constructor(loc, expression, shouldEvaluationTo) {
		super(
			`${JSON.stringify(expression)} has been used,
 it will evaluate to ${shouldEvaluationTo} in next major version`
		);
			...
			(compilation, { normalModuleFactory }) => {
				const handler = (parser, parserOptions) => {
					...
					if (localOptions.global !== false) {
						const withWarning = localOptions.global === null;
							...
							.tap("NodeStuffPlugin", expr => {
								...
								if (withWarning) {
									parser.state.module.addWarning(
										new NodeStuffInWebError(dep.loc, "global", "undefined")
									);
								}
							});
					}
