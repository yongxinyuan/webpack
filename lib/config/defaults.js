 ...
 * @param {Object} options options
 * @param {TargetProperties | false} options.targetProperties target properties
 * @param {Experiments} options.experiments experiments
 * @returns {void}
 */
const applyNodeDefaults = (node, { experiments, targetProperties }) => {
	if (node === false) return;

	const nextDefaults = experiments.futureDefaults;

	F(node, "global", () => {
		if (targetProperties && targetProperties.global) return false;
		// TODO webpack 6 should always default to false
		return nextDefaults ? "warn" : true;
	});
	F(node, "__filename", () => {
		if (targetProperties && targetProperties.node) return "eval-only";
		// TODO webpack 6 should always default to false
		return nextDefaults ? "warn-mock" : "mock";
	});
	F(node, "__dirname", () => {
		if (targetProperties && targetProperties.node) return "eval-only";
		// TODO webpack 6 should always default to false
